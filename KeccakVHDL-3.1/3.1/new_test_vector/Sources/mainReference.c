...
void displayAllInOneTest(unsigned int rate, unsigned int capacity)
{
    #define refLenMax 128
    ...
    printf("All-in-one test for Keccak[r=%d, c=%d]\n", rate, capacity);
    for(offset=0; offset<refLen; offset++)
...
{
    displayAllInOneTest(1152,  448);
    displayAllInOneTest(1088,  512);
    displayAllInOneTest(1024,  576);
    displayAllInOneTest( 832,  768);
    displayAllInOneTest( 576, 1024);
}


void displaySpongeIntermediateValues_vhdl()
{
    ...
    rate = 1600-capacity;    
    f = fopen("keccak_in.txt", "w");
	f2= fopen("keccak_ref_out.txt", "w");
    if (f == NULL)
        printf("Could not open %s\n", fileName);
	...
	{
		//gen random messages of increasing lenght with step of one bit
		messageLength=100+count_test;
		for(j=0;j<4000;j++)
			message[j]=0;
		for(j=0;j<(1+messageLength/8);j++)
			message[j]=(char) rand();

		mask=0xff >> (8-messageLength %8);
		...
		if (((messageLength + 1) % rate)==0 ) {
			for(j=0;j<(messageLength + 1 +rate)/rate;j++)
			{

				for(i=0;i<(rate/64);i++)
				{
					for(k=0;k<8;k++)
						fprintf(f,"%02X",padded_message[j*(8*rate/64)+i*8+(7-k)]);			
					fprintf(f,"\n");
				}
			}
		...
		else
		{
			...
			for(j=0;j<(messageLength+(rate - (messageLength%rate)))/rate;j++)
			{
...
int main()
{
	...
	displaySpongeIntermediateValues_vhdl();
	KeccakPermutationIntermediateValuesForVHDL();
